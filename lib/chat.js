...
/** 首轮提示词里“用户真正说的那句”的分隔标记；镜像里会从这里裁掉前面那段。 */
export const QUESTION_MARKER = '【我的问题】';

/**
 * 组装首轮的完整提示词：交接说明 + 用户那句话。
 * @param info - 论文信息。
 * @param question - 用户输入的问题（可空：空的时候就是让它先精读一遍）。
 * @returns 真正发给子代理的文本。
 */
export function combinedPrompt(info, question) {
  const head = firstPrompt(info);
  const asked = typeof question === 'string' ? question.trim() : '';
  if (asked.length > 0) return `${head}\n${asked}`;
  const cut = head.lastIndexOf(QUESTION_MARKER);
  const body = cut === -1 ? head : head.slice(0, cut).trimEnd();
  return `${body}\n\n请先通读全文，按上面六段给我一份中文精读总结。`;
}
  ...
  try {
    ...
    for (const event of events) {
      ...
      if (event.type === 'user/message') {
        ...
        // 首轮那条是我们自己拼的交接提示词：镜像里只留标记之后用户真正说的那句，
        // 没说的话就整条不显示（提示词不占聊天位置）。
        if (text.includes(QUESTION_MARKER)) {
          const asked = text.slice(text.lastIndexOf(QUESTION_MARKER) + QUESTION_MARKER.length).trim();
          if (asked.length === 0) continue;
          out.push({ role: 'user', text: asked, at: SeqTime(event) });
          continue;
        }
        if (isHandoffText(text)) continue;
...
/** 认一条“交接说明”文本（首轮提示词的前两句足够区分）。 */
function isHandoffText(text) {
  return text.startsWith('我要读下面这篇论文') || text.startsWith('【论文阅读上下文】');
}
