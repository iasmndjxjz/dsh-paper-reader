...
import { PaperSubagents, firstPrompt, MAX_CONTEXT_CHARS } from './chat.js';
...
export function apply(ctx) {
  ctx.inject(['webServer', 'connection'], (webCtx) => {
    ...
    /** POST chat：没有子代理就起一个；已经有了就直接把这句话发给它（人类权限通道）。 */
    const handleChatStart = async (req, res) => {
      ...
      // 已经有一条挂在这个会话下的子代理：这句话直接交给它（冷状态会被这条通道自动唤醒）。
      if (existing !== undefined && existing.parentSessionId === sessionId && message !== undefined) {
        try {
          const sent = await chats.send({ paper: target, sessionId, message, delivery: body.delivery });
          json(res, 200, {
            ok: true,
            path: target,
            childId: sent.childId,
            delivered: true,
            messageId: sent.messageId ?? null,
            chat: capability,
          });
        } catch (error) {
          fail(res, 500, 'send-failed', error instanceof Error ? error.message : String(error));
        }
        return;
      }
      ...
      try {
        ...
        if (ext === 'pdf' || TEXT_EXT.has(ext)) {
          try {
            ...
            // PDF 抽取结果落一份到插件自己的目录：交给子代理当兜底（不动用户文件）
            if (extracted.kind === 'pdf' && typeof extracted.text === 'string' && extracted.text.length > 0) {
              const mtime = Number.isFinite(info.mtimeMs) ? info.mtimeMs : 0;
              cachePath = await chats.writeCache(target, mtime, extracted.text);
            }
          } catch (error) {
            webCtx.logger?.warn?.(`paper-reader: 论文正文抽取失败（${error.message}）`);
          }
        }
        ...
        const started = await chats.ensure({
          paper: target,
          sessionId,
          prompt: message === undefined ? prompt : `${prompt}\n\n我这次想问的是：${message}`,
        });
        json(res, 200, {
          ok: true,
          path: target,
