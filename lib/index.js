 ...
 *   5. GET  chat        —— 论文对话历史；
 *   6. POST chat        —— 驱动真 Agent 会话，NDJSON 流式回话（右栏实时聊天）；
 *      POST chat/cancel —— 停下正在跑的一轮；
...
import { PaperChats, chatCapabilities, contextPreamble, MAX_CONTEXT_CHARS } from './chat.js';
...
export function apply(ctx) {
  ctx.inject(['webServer', 'connection'], (webCtx) => {
    ...
    /** 论文对话引擎（每篇论文一个真会话；懒创建）。 */
    const chats = new PaperChats({
      ctx: webCtx,
    ...
    /** GET chat：这篇论文的对话历史。 */
    const handleChatHistory = async (req, res, url) => {
      let target;
      try {
      ...
      const history = await chats.history(target);
      json(res, 200, {
        ok: true,
        path: target,
        sessionId: history.sessionId,
        messages: history.messages,
        chat: chatCapabilities(webCtx),
      });
    };

    /** POST chat/cancel：停下正在跑的一轮。 */
    const handleChatCancel = async (req, res) => {
      const body = await readJsonBody(req, res);
      if (body === undefined) return;
      const agents = webCtx.get('agents');
      const chatsApi = chats;
      try {
        const record = body.path === undefined ? undefined : await chatsApi.record(String(body.path));
        const agent = record === undefined ? undefined : agents?.get?.(record.sessionId);
        if (agent === undefined) {
          json(res, 200, { ok: true, cancelled: false });
          return;
        }
        agent.cancel('paper-reader');
        json(res, 200, { ok: true, cancelled: true });
      } catch (error) {
        fail(res, 500, 'cancel-failed', error.message);
      }
    };

    /** POST chat/reset：清空这篇论文的对话。 */
    const handleChatReset = async (req, res) => {
      ...
      if (live !== undefined) {
        try { live.cancel('paper-reader'); } catch { /* 忽略 */ }
      }
      await chats.reset(target);
      json(res, 200, { ok: true });
    };

    /** POST chat：驱动真 Agent，流式（NDJSON）回话。 */
    const handleChatTurn = async (req, res) => {
      const body = await readJsonBody(req, res);
      if (body === undefined) return;
      if (typeof body.message !== 'string' || body.message.trim().length === 0) {
        fail(res, 400, 'empty-message', '消息不能为空');
        return;
      }
      const roots = allowedRoots();
      let target;
      try {
        target = await resolveInside(body.path, roots);
      } catch (error) {
        fail(res, 400, 'bad-path', error.message);
        return;
      }
      ...
      const capability = chatCapabilities(webCtx);
      if (!capability.available) {
        fail(res, 503, 'agents-unavailable', '当前组合没有 agents 服务，无法开启论文对话');
        return;
      }
      if (capability.model === null) {
      ...
      try {
        ...
        // 首轮才需要注入论文上下文：文本文件给路径，PDF 直接给抽取正文。
        const history = await chats.history(target);
        const needsContext = history.messages.length === 0;
        let context;
        if (needsContext) {
          let extracted;
          try {
            if (ext === 'pdf' || TEXT_EXT.has(ext)) extracted = await paperTextOf(target, ext);
          } catch (error) {
          ...
          context = extracted?.kind === 'pdf' && typeof extracted.text === 'string' && extracted.text.length > 0
            ? `${preamble}\n\n===== 抽取正文开始 =====\n${extracted.text}\n===== 抽取正文结束 =====`
            : preamble;
        }
        ...
        writeEvent({
          ...
          contextInjected: context !== undefined,
        });
        const result = await chats.turn({
          paper: target,
          name: path.basename(target),
        ...
        writeEvent({ type: 'done', sessionId: result.sessionId, text: result.text, tools: result.tools });
        closed = true;
        res.end();
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (error?.name === 'AbortError' || controller.signal.aborted) {
          if (!res.headersSent) fail(res, 499, 'cancelled', '已取消');
          else {
        ...
        if (!res.headersSent) fail(res, 500, 'chat-failed', message);
        else {
    ...
    const route = async (req, res) => {
      ...
      try {
        ...
        if (sub === 'chat') {
          if (req.method === 'GET' || req.method === 'HEAD') return await handleChatHistory(req, res, url);
          if (req.method === 'POST') return await handleChatTurn(req, res);
          return fail(res, 405, 'method', '需要 GET 或 POST');
        }
        if (sub === 'chat/cancel') {
          if (req.method !== 'POST') return fail(res, 405, 'method', '需要 POST');
          return await handleChatCancel(req, res);
        }
        if (sub === 'chat/reset') {
          if (req.method !== 'POST') return fail(res, 405, 'method', '需要 POST');
