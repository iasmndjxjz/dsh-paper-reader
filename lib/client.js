...
`;
  ...
  function SubagentPane({ doc, getSessionId, openSubagent, dockPaper, canDock }) {
    ...
    /** 拉状态 + 镜像。 */
    const refresh = useCallback(async (showBusy) => {
    ...
    useEffect(() => {
        ...
        try {
          ...
          await readNdjson(response, (event) => {
            ...
            if (event.type === 'delta') {
              ...
              setState((previous) => ({ ...previous, messages: [...previous.messages, event.message] }));
            } else if (event.type === 'state') {
              setState((previous) => ({ ...previous, running: event.running === true, status: event.live === true ? (event.running === true ? 'running' : 'idle') : 'cold' }));
            } else if (event.type === 'idle') {
              if (acc.length > 0) {
                // 注意：setState 的 updater 是延迟执行的，必须先把文本取出来，
                // 否则读到的会是已经清空的累加变量（空气泡）。
                const finished = acc;
                acc = '';
                setLive('');
                setState((previous) => ({
                  ...
                  status: 'idle',
                  running: false,
                  messages: [...previous.messages, { role: 'assistant', text: finished }],
                }));
              }
            }
          });
      ...
      void run();
      return () => { cancelled = true; controller.abort(); streamRef.current = null; };
    }, [path, state.childId, getSessionId]);
    ...
    /** 起子代理（首轮带上下文，可带一句开场提问）。 */
    const start = useCallback(async (prompt, mode) => {
      if (doc === null) { setError('请先在左侧选一篇论文'); return; }
      const sessionId = getSessionId();
      if (typeof sessionId !== 'string' || sessionId.length === 0) {
        setError('没有找到当前会话：论文子代理需要挂在你的会话下');
        return;
      }
      setBusy(true);
      setError('');
      try {
        ...
        setNote(payload.reused === true ? '已复用这条论文子代理' : '子代理已启动：它的记录就在你当前会话下面');
      } catch (caught) {
        setError(caught.message);
      } finally {
        setBusy(false);
    ...
    /** 把一句话交给子代理：没有就走宿主启动，有了就交给原生输入框草稿。 */
    const ask = useCallback(async (prompt) => {
      if (state.address !== null && typeof openSubagent === 'function') {
        const ok = openSubagent(state.address, prompt);
        if (!ok) setError('没能切到原生子会话，请稍后再试');
        return;
      }
      await start(prompt, 'ask');
    }, [state.address, openSubagent, start]);

    /** 在原生子会话里打开（桌面同时把论文停靠到右侧栏）。 */
    const openNative = useCallback((prompt) => {
      if (state.address === null) {
        setError('还没有子代理，先点「精读总结」或「开始对话」');
        return;
      }
      if (typeof canDock === 'function' && canDock() && typeof dockPaper === 'function') dockPaper();
      const ok = typeof openSubagent === 'function' ? openSubagent(state.address, prompt) : false;
      if (!ok) setError('没能切到原生子会话，请稍后再试');
    ...
    }, [path, getSessionId, refresh]);

    const forget = useCallback(async () => {
      if (path.length === 0) return;
      try {
        await apiPost('chat/reset', { path });
        setState({ status: 'none', messages: [], childId: null, address: null, running: false });
        setLive('');
        setNote('已忘掉这条子代理：下次会重新起一个');
      } catch (caught) {
        setError(caught.message);
      }
    }, [path]);

    const statusText = state.status === 'running' ? '运行中'
      : state.status === 'idle' ? '空闲'
        : state.status === 'cold' ? '已休眠（点打开会恢复）'
          : '未开始';
    const badge = state.status === 'none' ? '·' : state.status === 'running' ? '●' : '○';
    const usable = capability === null || capability.available !== false;

    return h('div', { className: 'dpr-console' },
      h('div', { className: 'dpr-console-head' },
        h('span', { className: 'dpr-chat-head-title' }, IconChat ? h(IconChat, { size: 14 }) : null, '论文子代理'),
        h('span', { className: 'dpr-chip', title: state.childId ?? '' }, `${badge} ${statusText}`),
        capability?.provider !== undefined && capability?.provider !== null ? h('span', { className: 'dpr-chip' }, capability.provider) : null,
        h('span', { style: { flex: '1 1 auto' } }),
        state.running ? h('button', { className: 'dpr-btn', 'data-danger': 'true', onClick: interrupt, title: '按人类权限打断' }, '停止') : null,
        state.childId !== null ? h('button', { className: 'dpr-btn', onClick: forget, title: '忘掉映射（下次重起）' }, '重置') : null),

      !usable ? h('div', { className: 'dpr-error' }, '当前组合没有子代理服务（dsh-subagent），这个面板用不了。') : null,

      h('div', { className: 'dpr-console-actions' },
        h('button', {
          className: 'dpr-card',
          disabled: doc === null || busy,
          onClick: () => void start(undefined, 'summary'),
        },
          h('span', { className: 'dpr-card-title' }, IconSpark ? h(IconSpark, { size: 13 }) : null, '精读总结'),
          h('span', { className: 'dpr-card-desc' }, '按「结论/问题/方法/结果/局限/术语」六段精读')),
        h('button', {
          className: 'dpr-card',
          disabled: doc === null || busy,
          onClick: () => openNative(SUMMARY_PROMPT),
        },
          h('span', { className: 'dpr-card-title' }, IconChat ? h(IconChat, { size: 13 }) : null, '在原生会话里打开'),
          h('span', { className: 'dpr-card-desc' }, canDock?.() === true ? '论文停靠右侧栏，中间原生聊天' : '切到原生子会话继续聊'))),

      state.childId !== null ? h('div', { className: 'dpr-note' }, `子代理：${String(state.childId).slice(0, 22)}…（记录在你当前会话下）`) : null,

      h('div', { className: 'dpr-console-msgs', ref: scrollRef, onScroll: () => {
        const node = scrollRef.current;
        if (node !== null) stickRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 80;
      } },
        state.messages.length === 0 && live.length === 0
          ? h('div', { className: 'dpr-empty' },
            state.childId === null
              ? (doc === null ? '先在左侧选一篇论文。' : '还没开始。点「精读总结」，或直接问一句——我会在你的会话下开一个论文子代理。')
              : '镜像还是空的：子代理在原生会话里回复后这里会同步。')
          : null,
        state.messages.map((message, index) => h('div', {
          key: `${index}-${message.at ?? ''}`,
          className: 'dpr-mirror',
          ...
          Array.isArray(message.tools) && message.tools.length > 0
            ...
            : null,
          h('div', { className: 'dpr-bubble dpr-md' }, h(MarkdownView, { text: message.text })))),
        live.length > 0
          ? h('div', { className: 'dpr-mirror', 'data-role': 'assistant' },
            h('div', { className: 'dpr-bubble dpr-md' }, h(MarkdownView, { text: live }), h('span', { className: 'dpr-caret' })))
          : null),

      error.length > 0 ? h('div', { className: 'dpr-error' }, error) : null,
      note.length > 0 ? h('div', { className: 'dpr-note' }, note) : null,

      h('div', { className: 'dpr-quick' },
      ...
      doc !== null ? h('div', { className: 'dpr-info' },
        h('div', { className: 'dpr-info-row' }, h('span', { className: 'dpr-info-key' }, '文件'), h('span', { className: 'dpr-info-val' }, doc.name)),
        h('div', { className: 'dpr-info-row' }, h('span', { className: 'dpr-info-key' }, '路径'), h('span', { className: 'dpr-info-val' }, doc.path)),
        h('div', { className: 'dpr-info-row' }, h('span', { className: 'dpr-info-key' }, '大小'), h('span', { className: 'dpr-info-val' }, `${sizeText(doc.size)} · ${timeText(doc.mtime)}`))) : null);
  }
