...
`;
  ...
  // -------------------------------------------------- 右栏：实时对话 ----
  /**
   * 论文实时对话：随时可打字，宿主侧跑真 Agent（工具 / 读写文件 / 持久化都是原生的）。
   * 「精读总结」只是往对话里发一条指令，不是进入聊天的前置条件。
   */
  function ChatPane({ doc, canAsk, askAgent, onBusyChange }) {
    const [messages, setMessages] = useState([]);
    const [input, setInput] = useState('');
    const [busy, setBusy] = useState(false);
    const [error, setError] = useState('');
    const [note, setNote] = useState('');
    const [meta, setMeta] = useState({ sessionId: '', model: null, loaded: false });
    ...
    useEffect(() => {
      ...
      setError('');
      setNote('');
      setMeta({ sessionId: '', model: null, loaded: false });
      if (path.length === 0) return () => { cancelled = true; };
      (async () => {
        try {
          const payload = await apiGet('chat', { path });
          if (cancelled) return;
          setMessages(Array.isArray(payload.messages) ? payload.messages : []);
          setMeta({
            sessionId: typeof payload.sessionId === 'string' ? payload.sessionId : '',
            model: payload.chat?.model ?? null,
            loaded: true,
          });
        } catch (caught) {
          if (cancelled) return;
    ...
    const stop = useCallback(() => {
      ...
      if (path.length > 0) void apiPost('chat/cancel', { path }).catch(() => {});
      setBusy(false);
    }, [path]);
    ...
    const send = useCallback(async (raw) => {
      ...
      setMessages((previous) => [
        ...
        { role: 'assistant', text: '', streaming: true, tools: [], at: Date.now() },
      ]);
      setBusy(true);
      const controller = new AbortController();
      abortRef.current = controller;
      /** 只改最后一条 assistant 气泡。 */
      const patchLast = (patch) => setMessages((previous) => {
        const next = [...previous];
      ...
      try {
        const response = await fetch(`${API}/chat`, {
          ...
          body: JSON.stringify({ path: doc.path, message: text }),
        ...
        await readNdjson(response, (event) => {
          if (event.type === 'start') {
            ...
            if (event.contextInjected === true) setNote('已把论文正文作为上下文交给模型（仅首个问题）');
          } else if (event.type === 'delta') {
            accumulated += event.text;
            patchLast({ text: accumulated });
          } else if (event.type === 'tool') {
            patchLast((message) => ({ ...message, tools: [...(message.tools ?? []), event.name] }));
          } else if (event.type === 'error') {
            failed = event.message ?? '生成失败';
          } else if (event.type === 'cancelled') {
            patchLast({ streaming: false, cancelled: true, text: accumulated });
          } else if (event.type === 'done') {
            accumulated = typeof event.text === 'string' && event.text.length > 0 ? event.text : accumulated;
            patchLast({ text: accumulated, streaming: false, tools: event.tools ?? [] });
          }
        });
        if (failed.length > 0) throw new Error(failed);
        patchLast({ streaming: false, text: accumulated });
      } catch (caught) {
        if (caught?.name === 'AbortError') {
          patchLast((message) => ({ ...message, streaming: false, cancelled: true }));
        } else {
          const detail = caught?.message ?? String(caught);
          setError(detail);
          patchLast((message) => ({
            ...message,
            streaming: false,
            error: true,
            text: message.text.length > 0 ? message.text : `⚠️ ${detail}`,
          }));
        }
      } finally {
        abortRef.current = null;
    ...
    const reset = useCallback(async () => {
      ...
      try {
        ...
        setMessages([]);
        setMeta((previous) => ({ ...previous, sessionId: '' }));
        setError('');
        setNote('已开始新对话：下一个问题会新建一个论文会话。');
      } catch (caught) {
        setError(caught.message);
      }
    }, [path, stop]);

    /** 把论文交给原生对话（保留原生 Agent 能力的入口）。 */
    const handoff = useCallback(() => {
      if (doc === null) return;
      const ok = askAgent(`请精读这篇论文：${doc.path}\n先用 read 工具读原文，然后给出：\n1) 研究问题与动机；\n2) 方法与技术路线；\n3) 关键实验/结论与具体数据；\n4) 局限、可疑之处和可复用的点。`);
      if (!ok) setError('当前会话没有可写的输入框，请先回到对话再试');
    }, [doc, askAgent]);
    ...
    return h('div', { className: 'dpr-chat' },
      h('div', { className: 'dpr-chat-head' },
        h('span', { className: 'dpr-chat-head-title' }, IconChat ? h(IconChat, { size: 14 }) : null, '论文对话'),
        meta.model !== null ? h('span', { className: 'dpr-chip', title: `论文会话：${meta.sessionId}` }, meta.model.model) : null,
        h('span', { style: { flex: '1 1 auto' } }),
        h('button', {
          className: 'dpr-btn',
          disabled: doc === null || busy,
          title: '让 Agent 通读全文并给一份中文精读总结',
          onClick: () => void send(SUMMARY_PROMPT),
        }, IconSpark ? h(IconSpark, { size: 12 }) : null, '精读总结'),
        h('button', {
          className: 'dpr-btn',
          disabled: messages.length === 0,
          title: '清空这篇论文的对话',
          onClick: () => void reset(),
        }, IconTrash ? h(IconTrash, { size: 12 }) : null, '新对话'),
        canAsk ? h('button', { className: 'dpr-btn', title: '把这篇文章写进原生输入框，回到主对话继续', onClick: handoff }, '问 Agent') : null),

      h('div', { className: 'dpr-msgs', ref: scrollRef, onScroll },
        !hasMessages && meta.loaded
          ? h(React.Fragment, null,
            h('div', { className: 'dpr-empty' },
              doc === null ? '先在左侧选一篇论文。' : '随时可以提问——这是一个真正的 Agent 会话，它自己能读文件、查资料、跑命令。',
              doc === null ? null : h('div', { style: { marginTop: '6px' } }, '想先要一份总结就点上面的「精读总结」，也可以直接在下面问。')),
            doc === null ? null : h('div', { className: 'dpr-quick' }, QUICK_PROMPTS.map((prompt) => h('button', {
              key: prompt,
        ...
        messages.map((message, index) => {
          ...
          return h('div', {
            ...
            isUser
              ? h('div', { className: 'dpr-bubble' }, message.text)
              : h('div', { className: 'dpr-bubble dpr-md' },
                tools.length > 0 ? h('div', { className: 'dpr-msg-tools' },
                  tools.map((tool, toolIndex) => h('span', { key: `${tool}-${toolIndex}`, className: 'dpr-tool' },
                    IconSpark ? h(IconSpark, { size: 10 }) : null, tool))) : null,
                message.text.length > 0 ? h(MarkdownView, { text: message.text }) : null,
                message.streaming === true ? h('span', { className: 'dpr-caret' }) : null,
                message.cancelled === true && message.text.length === 0 ? '（已停止）' : null));
        }),
      ...
      h('div', { className: 'dpr-composer' },
        h('textarea', {
          ref: textRef,
          rows: 1,
          placeholder: doc === null ? '先选一篇论文…' : '问这篇论文的任何问题（Enter 发送，Shift+Enter 换行）',
          value: input,
          disabled: doc === null,
        ...
        h('div', { className: 'dpr-composer-row' },
          h('span', { className: 'dpr-composer-hint' },
            busy ? 'Agent 正在处理…' : (meta.model === null ? '' : `${meta.model.model} · 可读写文件、跑命令`)),
          busy
            ? h('button', { className: 'dpr-btn', 'data-danger': 'true', onClick: stop }, IconStop ? h(IconStop, { size: 11 }) : null, '停止')
            : h('button', {
              className: 'dpr-btn',
              'data-primary': 'true',
  ...
  function PaperPanel(props) {
    ...
    const chatPane = h('div', { className: 'dpr-pane', style: narrow && tab !== 'chat' ? { display: 'none' } : undefined },
      h(ChatPane, { doc, canAsk, askAgent, onBusyChange: setChatBusy }));
    ...
    return h('div', { className: 'dpr-root', 'data-paper-reader': 'ready' },
      ...
      narrow ? h('div', { className: 'dpr-tabs' },
        ...
        h('button', { className: 'dpr-tab', 'data-active': tab === 'chat' ? 'true' : undefined, onClick: () => setTab('chat') },
          '对话', chatBusy ? h('span', { className: 'dpr-dot' }) : null)) : null,
