...
.dpr-body{flex:1 1 auto;min-height:0;display:grid;grid-template-columns:240px minmax(0,1fr) 400px}
.dpr-body[data-narrow="true"]{grid-template-columns:minmax(0,1fr)}
.dpr-resize{position:relative;width:0;z-index:2;cursor:col-resize}
.dpr-resize::after{content:'';position:absolute;top:0;bottom:0;left:-3px;width:6px}
.dpr-resize:hover::after{background:var(--dsw-alias-border-l3)}
...
.dpr-paper-inner{max-width:min(1180px,100%);margin:0 auto;padding:26px 30px 140px;font-size:15.5px;line-height:1.85;
...
`;
  ...
  function PaperPanel(props) {
    ...
    const [leftW, setLeftW] = useState(Number.isFinite(stored.leftW) ? stored.leftW : 240);
    const [rightW, setRightW] = useState(Number.isFinite(stored.rightW) ? stored.rightW : 400);
    ...
    /** 持久化需要跨面板切换保留的少量状态。 */
    useEffect(() => {
      saveStored({ root, showAll, path, tab, leftW, rightW, expanded: [...expanded].slice(0, 200) });
    }, [root, showAll, path, tab, leftW, rightW, expanded]);

    /**
     * 桌面铺满：进门时把左侧会话栏收起来，离开时按原样还回去。
     * 只在“确实是宽的、而且当时是展开的”时候动手，避免误改用户布局。
     */
    useEffect(() => {
      if (typeof props.toggleSidebar !== 'function') return undefined;
      if (typeof window !== 'undefined' && window.innerWidth < NARROW_WIDTH) return undefined;
      let toggled = false;
      try {
        ...
        for (let depth = 0; depth < 10 && node !== null; depth += 1) {
          ...
          if (node !== null && node.querySelector('[data-rightbar-col]') !== null) { frame = node; break; }
        }
      ...
      return () => {
        if (!toggled) return;
        try { props.toggleSidebar(); } catch { /* 已经关掉就算了 */ }
      };
    }, [props.toggleSidebar]);

    /** 面板实际宽度决定平铺还是页签（桌面窗口小也不会误退成页签）。 */
    useEffect(() => {
      const el = rootRef.current;
      const measure = () => {
        const width = el === null || el.clientWidth === 0 ? window.innerWidth : el.clientWidth;
    ...
    const startResize = useCallback((side) => (event) => {
      ...
      const base = side === 'left' ? leftW : rightW;
      const move = (moveEvent) => {
        const delta = moveEvent.clientX - startX;
        if (side === 'left') setLeftW(Math.round(Math.min(460, Math.max(150, base + delta))));
        else setRightW(Math.round(Math.min(720, Math.max(280, base - delta))));
      };
      const done = () => {
    ...
    return h('div', { className: 'dpr-root', 'data-paper-reader': 'ready', ref: rootRef },
      h('div', { className: 'dpr-bar' },
      ...
      h('div', {
        ...
        style: narrow ? undefined : { gridTemplateColumns: `${leftW}px minmax(0,1fr) ${rightW}px` },
      },
        treePane,
        narrow ? null : h('div', { className: 'dpr-resize', title: '拖动调整目录宽度', onPointerDown: startResize('left') }),
        paperPane,
        narrow ? null : h('div', { className: 'dpr-resize', title: '拖动调整对话宽度', onPointerDown: startResize('right') }),
        chatPane));
  }
